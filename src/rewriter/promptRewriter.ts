...
export interface RewriteOptions {
  ...
  userLevel?: UserLevel; // User experience level for persona-based prompts
...
export class PromptRewriter {
  ...
  public async processPrompt(prompt: string): Promise<RewriteWorkflowResult> {
    try {
      ...
      // Step 3: If user prefers Groq, skip VS Code LM
      if (this.preferredModel === 'groq') {
        if (this.groqRewriter) {
          const rewrite = await this.groqRewriter.enhancePrompt(prompt);
          return {
            shouldRewrite: true,
            analysis,
            rewrite,
      ...
      // Step 4: Try VS Code Language Model first (for auto, gpt-4, claude preferences)
      const vsCodeLmAvailable = await isVsCodeLmAvailable();
      if (vsCodeLmAvailable) {
        try {
          const rewrite = await this.vscodeLmRewriter.enhancePrompt(prompt);
          return {
            shouldRewrite: true,
            analysis,
            rewrite,
      ...
      // Step 6: No rewriter available
      return {
        shouldRewrite: false,
        analysis,
        error: 'No AI model available. Please enable Copilot/Claude Code or configure a Groq API key.',
      };
    } catch (error) {
      return {
        shouldRewrite: false,
