...
export function analyzePrompt(prompt: string): AnalysisResult {
  ...
  const hasVagueVerb = VAGUE_VERBS.some((verb: string) => {
  ...
  const hasLearningVerb = LEARNING_VERBS.some((verb: string) => {
    const regex = new RegExp(`\\b${verb}\\b`, 'i');
    return regex.test(trimmed);
  });

  // Only flag learning verbs if they lack context patterns
  const hasContext = CONTEXT_PATTERNS.some((pattern: RegExp) => pattern.test(trimmed));
  ...
  const hasBroadTerms = BROAD_TERMS.some((term: string) => {
