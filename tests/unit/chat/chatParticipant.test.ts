...
// Mock SecretStorage - must be before vscode mock
const mockGetGroqApiKey = jest.fn().mockResolvedValue('');
jest.mock('../../../src/utils/secretStorage', () => ({
  getSecretStorage: jest.fn(() => ({
    getGroqApiKey: mockGetGroqApiKey,
  })),
}));
...
describe('Chat Participant', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    mockGetGroqApiKey.mockResolvedValue(''); // Reset to empty key
    ...
    // Setup mock config (note: groqApiKey is now in SecretStorage, not config)
    mockConfig = {
