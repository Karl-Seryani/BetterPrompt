...
describe('PromptRewriter', () => {
  ...
  describe('cancellation support', () => {
    it('should accept an optional cancellation token parameter', async () => {
      const rewriter = new PromptRewriter(mockOptions);
      // Should compile and run without errors when token is undefined
      const result = await rewriter.processPrompt('make a website', undefined);
      expect(result).toBeDefined();
    });

    it('should return error when cancellation is requested during processing', async () => {
      // Create a mock cancellation token that is already cancelled
      const mockToken = {
      ...
      const rewriter = new PromptRewriter(mockOptions);
      const result = await rewriter.processPrompt('make a website', mockToken as any);
